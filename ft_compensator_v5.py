...
class FTCompensatorLPFMadgwick(object):
    def __init__(self):
        rospy.init_node('ft_compensator_lpf_madgwick', anonymous=False)
    ...
    def imu_cb(self, msg):
        """Process IMU data: LPF first, then Madgwick"""
        ...
        self.q_madgwick = self.madgwick.update(self.gyro_filtered, self.accel_filtered)
    ...
    def update(self):
        if self.w_filtered is None or self.accel_filtered is None:
            return
        ...
        # Extract rotation from Madgwick orientation
        r_fused  = R.from_quat([self.q_madgwick[1], self.q_madgwick[2],
                                self.q_madgwick[3], self.q_madgwick[0]])
    ...
    def print_stats(self):
        ...
        print("\n" + "="*70)
        print("FINAL STATISTICS - Z-AXIS COMPENSATION (LPF + Madgwick)")
        print("="*70)
