...
class FTCompensatorLPFArUco(object):
    def __init__(self):
        rospy.init_node('ft_compensator_lpf_aruco', anonymous=False)
        ...
        # State
        self.q_aruco_target = None
        self.q_smoothed = None     
    ...
    def aruco_cb(self, msg):
        ...
        q_new = np.array([q.w, q.x, q.y, q.z])
        
        if self.q_aruco_target is not None and np.dot(self.q_aruco_target, q_new) < 0:
            q_new = -q_new
            
        self.q_aruco_target = q_new
    ...
    def update(self):
        if self.w_filtered is None or self.accel_filtered is None or self.gyro_filtered is None:
            return

        # Check ArUco freshness
        aruco_active = False
        if self.q_aruco_target is not None and self.last_aruco_time is not None:
        ...
        # Smooth orientation via temporal SLERP toward latest ArUco target
        if self.q_smoothed is None:
            self.q_smoothed = self.q_aruco_target.copy()
        else:
            self.q_smoothed = perform_slerp(
                self.q_smoothed, self.q_aruco_target, self.slerp_alpha)

        # Extract gravity from smoothed ArUco orientation
        r_fused  = R.from_quat([self.q_smoothed[1], self.q_smoothed[2],
                                self.q_smoothed[3], self.q_smoothed[0]])
    ...
    def print_stats(self):
        ...
        print("\n" + "="*70)
        print("FINAL STATISTICS - Z-AXIS COMPENSATION (LPF + ArUco)")
        print("="*70)
